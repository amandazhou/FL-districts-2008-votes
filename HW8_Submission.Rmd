...
```
Blacks are underrepresented in Florida.
...
```
Hispanics in Florida are have the same congressional representation as blacks, despite making up a larger percentage of the population.
...
```
Summed over all residents (in a direct democracy), McCain would have missed a majority to win the state of Florida but with the congressional district system, he was able to win in enough of them to tip the State towards him.
...
ggplot(data = q6, aes(x=hispshare,y=McCainShare)) + geom_point() + geom_smooth() +theme_bw()
```
...
ggplot(data = q7, aes(x=blackshare,y=turnout)) + geom_point() + geom_smooth() + scale_y_continuous(breaks= seq(0,1,.1), limits = c(0,1) ) +theme_bw()
```
